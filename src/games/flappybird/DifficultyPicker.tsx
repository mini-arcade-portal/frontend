...
export function DifficultyPicker() {
  ...
  const handleStart = () => {
    if (!selected) return
    navigate(`/play/flappybird?difficulty=${selected}`)
  }
      ...
      <div className="card-playful p-10 mb-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-ink-2 font-medium mt-3 max-w-lg">
            A nehezebb szint több pontot ér — de gyorsabbak a csövek és
            szűkebb a rés.
          </p>
        </div>
      </div>
            ...
            >
              ...
              <div className="space-y-1 font-mono text-xs">
                <div className="flex justify-between">
                  <span className="opacity-70">Sebesség:</span>
                  <span className="font-bold">{config.scrollSpeed}px/s</span>
                </div>
                <div className="flex justify-between">
                  <span className="opacity-70">Rés:</span>
                  <span className="font-bold">{config.pipeGap}px</span>
                </div>
              </div>
            </button>
          )
